########################################################################################################
#This code was is created by Celeste Venolia in August 2018 for Venolia et al. (2020). 
#deSolve structured sugar kelp DEB model
#Modified by Malene Dekke Eik in 2024 to assimilate higher growth rate in younger kelp individuals, and allometric area to weight. 
########################################################################################################
...
rates_Lo <- function(t, state, parameters, forcing) { 
  
  with(as.list(c(state, parameters, forcing)), { 
    
    # Check if T_field exists
    if (!exists("T_field")) stop("T_field is missing from forcing.", print(forcings))
    #Check if M_V exists 
    if (!exists("M_V")) stop("M_V is missing from state", print(state))
    
    acc_factor <- 1 #so we can plot the acc factor
    ### Adjust parameters for early growth function 
    if( W <= a_dw ){
      acc_factor <- 1 + (a_f-1) * (1-log(1+W)/log(1+a_dw))
      #acc_factor <- 1 + (log(a_f) / log(1 + a_dw)) * log(1 + W) #Calculating acceleration at this W
      
      #Adjusting parameters by acceleration factor 
      JENAM <- JENAM * acc_factor
      JECAM <- JECAM * acc_factor
      kE_C <- kE_C * acc_factor
      kE_N <- kE_N * acc_factor
      rho_PSU <- rho_PSU * acc_factor
      b_I <- min(1, (b_I * acc_factor))
    }
    ...
    J_CO2 <- J_CO2_M*(CO2_field[t]/(CO2_field[t]+K_C)) #J_CO2 units: molC/molM_V/h
    
    #Specific relaxation rate
    J_I <- (rho_PSU * I_field[t] * b_I * alpha)/(1+I_field[t] * b_I * alpha/(k_I * C_T)) #Unit: mol?? molM_V???1 h???1
    ...
    #Allometric relationship between dry weight(g) and thallus area (cm2) from calculations
    Area_allometric <- 39.3790 * (W/DM)^0.912530
    ...
    #output
    return(list(m_EC = dm_ECdt, m_EN = dm_ENdt, M_V = dM_Vdt, r=r, W=W, C_T=C_T, I=I, N=N, J_CO2=J_CO2, J_I=J_I, J_EC_A=J_EC_A, J_EN_A=J_EN_A, J_EC_C=J_EC_C, J_EN_C=J_EN_C, J_EN_M=J_EN_M, J_EC_M=J_EC_M, J_EN_G=J_EN_G, J_EC_G=J_EC_G, J_G=J_G, J_VM=J_VM, J_EC_R=J_EC_R, J_EN_R=J_EN_R, CO_2=CO2_field[t], L_allometric=L_allometric, Area_allometric = Area_allometric, J_O=J_O, info=rho_PSU, acc_factor = acc_factor))
  })
}
